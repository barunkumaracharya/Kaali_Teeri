...
from .models import Card, GameOf4x32, Hand, Round
...
game_4x32 = Blueprint('4x32', __name__, url_prefix='/4x32')


@game_4x32.route('/players')
@login_required
def list_players():
	...
	return render_template('4x32/players.html', team_1=team_1, team_2=team_2, present=present, present1=present_in_1, present2=present_in_2)


@game_4x32.route('/add_me/<team>')
@login_required
def add_player(team):
	global game_started, players
	if game_started == True:
		flash('Game Started wait for game to finish')
		return redirect(url_for('game_4x32.list_players'))
...
def start_game():
	...
	if len(team_1) == len(team_2):
		if game_started == False:
			game_started = True
			distribute_cards(8,0)
			setup_game()
			return redirect(url_for('game_4x32.choose_bid', chance=1))
		else:
			return render_template('4x32/message_popup.html')
	else:
		return render_template('4x32/message_popup.html')


@game_4x32.route('/game_query')
@login_required
def game_query():
	global game_started, team_1, team_2
...
@game_4x32.route('/check_selection')
@login_required
def check_selection():
	global bid_winner, player_bids
	if bid_winner != "":
		return "true"
	elif len(player_bids) == 2*len(team_1):
...
def choose_bid():
	...
	#TODO The player should only be shown the second set of hands once he has completed his first bidding process.
	hand = get_hand(game.phase)
	return render_template('/choose.html', suits=suits, number_of_partners=number_of_partners, cards=sorted(hand.cards, key=lambda x:(x.suit, x.value)), activityClass="inactiveLink",phase=game.phase)


@game_4x32.route('/bid', methods=['POST'])
@login_required
def post_choose_bid():
	global ordering_factor, player_order, game, players, rounds, player_points, bid_winner, bid_winner_index, player_bids

	bid_participation = request.form.get('bidParticipation')
	if bid_participation == "yes":
		if game.bidder == "":
			trump = request.form.get('trump')
			game.trump = trump
			ordering_factor = request.form.get('ordering')
			game.bid = request.form.get('no_of_hands')
			game.bidder = current_user.name
			bidder_index = player_order.index(game.bidder)

			temp1 = player_order[bidder_index:]
			temp1.extend(player_order[:bidder_index])

			player_order = temp1
			bid_winner = game.bidder
			bid_winner_index = bidder_index
			rounds.append(Round(starting_player=game.bidder, cards=[]))
	
	player_bids.append(current_user.name)


@game_4x32.route('/next_turn/<int:previos_player>/<int:round_id>')
@login_required
...
@game_4x32.route('/round/<int:round_id>')
@login_required
...
@game_4x32.route('/make_move/<string:suit>/<string:value>/<int:round_id>')
@login_required
...
@game_4x32.route('/display_results')
@login_required
def display_results():
...
def get_hand(phase):
	...
	if phase == 1:
		return Hand(player=current_user.name, cards=hand.cards[0:4])
	elif phase == 2:
		return Hand(player=current_user.name, cards=hand.cards[4:8])
	else:
		return hand
...
def distribute_cards(no_of_cards, offset):
	...
	for i in range(number_of_players):
		hands.append(Hand(player=players[i], cards=cards[distributed:distributed+no_of_cards]))
		distributed += no_of_cards